...
- **THEN** the system atomically: (1) inserts a `TagEvent` retiring the old tag with the selected reason and current timestamp, (2) updates `Asset.nfcTagId` to the new tag ID, and (3) inserts a new `TagEvent` attaching the new tag — all within a single Room transaction
